...
final class QRCodeGeneratorTests: XCTestCase {
    ...
    func testVersion1() throws {
        let r = try XCTUnwrap(QRCodeGenerator.generate(text: "Hi", version: 1, maskPattern: -1, ecl: ErrorCorrectionLevel.M.cValue))
        XCTAssertEqual(r.version, 1)
        XCTAssertEqual(r.matrix.width, 21)
    }

    func testVersion2() throws {
        let r = try XCTUnwrap(QRCodeGenerator.generate(text: "Hi", version: 2, maskPattern: -1, ecl: ErrorCorrectionLevel.M.cValue))
        XCTAssertEqual(r.matrix.width, 25)
    }

    func testVersion40() throws {
        let r = try XCTUnwrap(QRCodeGenerator.generate(text: "test", version: 40, maskPattern: -1, ecl: ErrorCorrectionLevel.L.cValue))
    ...
    func testAutoVersionPicksSmallest() throws {
        ...
        let r = try XCTUnwrap(QRCodeGenerator.generate(text: "Hi", version: 0, maskPattern: -1, ecl: ErrorCorrectionLevel.M.cValue))
        XCTAssertEqual(r.version, 1)
    }
    ...
    func testWidthMatchesVersionFormula() throws {
        for v in [1, 5, 10, 20, 40] {
            let r = try XCTUnwrap(
                QRCodeGenerator.generate(text: "test", version: v, maskPattern: -1, ecl: ErrorCorrectionLevel.L.cValue),
                "version \(v)"
            )
    ...
    func testAllMaskPatternsProduceResults() {
        for mask in 0...7 {
            XCTAssertNotNil(
                QRCodeGenerator.generate(text: "test", version: 1, maskPattern: mask, ecl: ErrorCorrectionLevel.M.cValue),
                "mask \(mask)"
            )
        }
    }
    ...
    func testAutoMaskPattern() {
        XCTAssertNotNil(
            QRCodeGenerator.generate(text: "test", version: 0, maskPattern: -1, ecl: ErrorCorrectionLevel.M.cValue)
        )
    }
    ...
    func testAllECLevels() {
        for ecl in ErrorCorrectionLevel.allCases {
            XCTAssertNotNil(
                QRCodeGenerator.generate(text: "test", version: 0, maskPattern: -1, ecl: ecl.cValue),
                "ECL \(ecl)"
            )
        }
    }
    ...
    func testEmptyStringReturnsNil() {
        XCTAssertNil(
            QRCodeGenerator.generate(text: "", version: 0, maskPattern: -1, ecl: ErrorCorrectionLevel.M.cValue)
        )
    }

    func testURLText() throws {
        let r = try XCTUnwrap(
            QRCodeGenerator.generate(text: "https://example.com", version: 0, maskPattern: -1, ecl: ErrorCorrectionLevel.M.cValue)
        )
    ...
    func testUTF8WithEmoji() throws {
        let r = try XCTUnwrap(
            QRCodeGenerator.generate(text: "Hello 🌍", version: 0, maskPattern: -1, ecl: ErrorCorrectionLevel.M.cValue)
        )
        XCTAssertGreaterThan(r.matrix.width, 0)
    }
    ...
    func testModuleMatrixIsSquare() throws {
        let r = try XCTUnwrap(
            QRCodeGenerator.generate(text: "test", version: 1, maskPattern: -1, ecl: ErrorCorrectionLevel.M.cValue)
        )
